...
try
{
    ...
    await ExecuteAsync(connection, $"""
        ...
        VALUES (1, 'native default', 60, 14, 3, 7, true, 7, true,
            1000, 5, 0, now(), now(), true);
    ...
    await using (var dataSource = NpgsqlDataSource.Create(storeConnectionString))
    {
        ...
        await ExerciseMediaRuntimeStoreAsync(new NpgsqlMediaRuntimeStore(dataSource));
    ...
    Console.WriteLine("Native persistence probe passed: reads, writes, rollback, concurrency, retry, PostgreSQL mappings, and the global-settings, IP-filter, provider/credential, model-routing, virtual-key, media, async-task, and request-log runtime stores.");
    return 0;
}
finally
{
...
static async Task ExerciseMediaRuntimeStoreAsync(IMediaRuntimeStore store)
{
    var active = new MediaRuntimeRecord
    {
        Id = Guid.NewGuid(),
        StorageKey = "native/media.png",
        VirtualKeyId = 41,
        MediaType = "image",
        ...
        Prompt = "native media persistence",
        StorageUrl = "s3://native/media.png",
        CreatedAt = DateTime.UtcNow
    };
    Ensure(await store.CreateAsync(active) == active.Id, "media runtime create failed");
    var replay = new MediaRuntimeRecord
    {
        StorageKey = active.StorageKey,
        VirtualKeyId = active.VirtualKeyId,
        MediaType = active.MediaType,
        SizeBytes = 999,
        Provider = "replayed-provider"
    };
    Ensure(await store.CreateAsync(replay) == active.Id && replay.Id == active.Id,
        "media runtime idempotent storage-key replay failed");
    Ensure((await store.GetByStorageKeyAsync(active.StorageKey))?.VirtualKeyId == 41,
        "media runtime storage-key lookup failed");

    var deleted = new MediaRuntimeRecord
    {
        Id = Guid.NewGuid(),
        StorageKey = "native/deleted.png",
        VirtualKeyId = 41,
        MediaType = "image",
        SizeBytes = 16,
        CreatedAt = DateTime.UtcNow,
    ...
    Ensure(await store.GetByStorageKeyAsync(deleted.StorageKey) is null,
        "media runtime soft-delete filter failed");
    Ensure((await store.GetByStorageKeyAsync(deleted.StorageKey, includeDeleted: true))?.DeletedAt is not null,
        "media runtime tombstone lookup failed");

    Ensure(await store.UpdateAccessStatsAsync(active.Id), "media runtime access update failed");
    Ensure((await store.GetByStorageKeyAsync(active.StorageKey))?.AccessCount == 1,
        "media runtime access counter failed");
    Ensure((await store.GetByVirtualKeyIdAsync(41)).Count == 1,
        "media runtime ownership listing failed");

    var aggregate = await store.GetAggregateStorageStatsAsync(7);
    Ensure(aggregate.TotalFiles == 1 && aggregate.TotalSizeBytes == 64,
        "media runtime active aggregate failed");
    Ensure(aggregate.ByProvider.TryGetValue("native-provider", out var providerSize) && providerSize == 64,
        "media runtime provider aggregate failed");

    var quota = await store.GetQuotaSnapshotAsync(41);
    Ensure(quota is { TotalFiles: 2, TotalSizeBytes: 80, MaxStorageSizeBytes: 1000, MaxFileCount: 5 },
        "media runtime quota snapshot failed");
    var usage = await store.GetGroupQuotaUsagesAsync(7);
    Ensure(usage.Count == 1 && usage[0].VirtualKeyGroupName == "native runtime group" &&
        usage[0].TotalFiles == 2 && usage[0].TotalSizeBytes == 80,
        "media runtime group quota reporting failed");
}
