    ...
    public class ProviderModelDiscovery : IProviderModelDiscovery
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex)
            {
                ...
                // For providers with static models, try to load them
                if (provider.ProviderType == ProviderType.SambaNova)
                {
                    return await LoadSambaNovaStaticModelsAsync(provider, cancellationToken);
                }
                else if (provider.ProviderType == ProviderType.MiniMax)
                {
                    return await LoadMiniMaxStaticModelsAsync(provider, cancellationToken);
                }
        ...
        private async Task<List<DiscoveredModel>> LoadSambaNovaStaticModelsAsync(Provider provider, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogDebug("Loading static SambaNova models");
                ...
                {
                    _logger.LogWarning("SambaNova models JSON file not found at {Path}", jsonPath);
                ...
                {
                    ModelId = m.Id,
                    Provider = provider.ProviderName ?? "sambanova",
                    DisplayName = m.Name ?? m.Id,
                    Capabilities = new CoreModelCapabilities
                    {
            ...
            {
                _logger.LogError(ex, "Failed to load static SambaNova models");
                return GetFallbackSambaNovaModels(provider);
            }
        }

        private List<DiscoveredModel> GetFallbackSambaNovaModels(Provider provider)
        {
            var models = new[]
            {
        ...
        private async Task<List<DiscoveredModel>> LoadMiniMaxStaticModelsAsync(Provider provider, CancellationToken cancellationToken)
        {
            try
            {
                var assembly = Assembly.GetExecutingAssembly();
                var assemblyLocation = assembly.Location;
                var directory = Path.GetDirectoryName(assemblyLocation);
                var jsonPath = Path.Combine(directory!, "StaticModels", "minimax-models.json");

                if (!File.Exists(jsonPath))
                {
                    _logger.LogWarning("MiniMax models JSON file not found at {Path}", jsonPath);
                    return new List<DiscoveredModel>();
                }
                ...
                {
                    ModelId = m.Id,
                    Provider = provider.ProviderName ?? "minimax",
                    DisplayName = m.Name ?? m.Id,
                    Capabilities = new CoreModelCapabilities
                    {
                        Chat = m.Capabilities?.Chat ?? false,
                        ChatStream = m.Capabilities?.Chat ?? false,
                        Vision = m.Capabilities?.Vision ?? false,
                        ImageGeneration = m.Capabilities?.ImageGeneration ?? false,
                        VideoGeneration = m.Capabilities?.VideoGeneration ?? false,
            ...
            {
                _logger.LogError(ex, "Failed to load static MiniMax models");
                return new List<DiscoveredModel>();
            }
        }
