...
# Integration Test Runner for Conduit - SambaNova Provider
# This script runs the SambaNova integration tests
...
echo -e "${BLUE}${BOLD}========================================${NC}"
echo -e "${BLUE}${BOLD}  SambaNova Integration Test Runner     ${NC}"
echo -e "${BLUE}${BOLD}========================================${NC}"
echo
...
PROVIDER_CONFIG="Config/providers/sambanova.yaml"
...
if [ ! -f "$PROVIDER_CONFIG" ]; then
    ...
    if [ -f "Config/providers/sambanova.template.yaml" ]; then
        cp Config/providers/sambanova.template.yaml "$PROVIDER_CONFIG"
        echo -e "${GREEN}✓ Created $PROVIDER_CONFIG${NC}"
        echo -e "${RED}   ⚠️  You MUST edit this file and add your SambaNova API key!${NC}"
        echo -e "${RED}   Edit: $TEST_DIR/$PROVIDER_CONFIG${NC}"
        echo -e "${RED}   Look for: apiKey: \"YOUR_SAMBANOVA_API_KEY_HERE\"${NC}"
        exit 1
    else
        echo -e "${RED}❌ Template file not found: Config/providers/sambanova.template.yaml${NC}"
        exit 1
    fi
fi
...
if grep -q "YOUR_SAMBANOVA_API_KEY_HERE" "$PROVIDER_CONFIG" 2>/dev/null; then
    ...
    echo -e "${RED}   Edit: $TEST_DIR/$PROVIDER_CONFIG${NC}"
    echo -e "${RED}   Replace: YOUR_SAMBANOVA_API_KEY_HERE with your actual API key${NC}"
    exit 1
fi
...
# Run the SambaNova tests
echo -e "${BLUE}🧪 Running SambaNova integration tests...${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
if [ $TEST_EXIT_CODE -eq 0 ]; then
    echo -e "${GREEN}${BOLD}✅ All SambaNova tests passed!${NC}"
