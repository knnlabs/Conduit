...
var outputFilename = args.Length > 0 ? args[0] : "cerebras-models.sql";

Console.WriteLine("=== Cerebras Models SQL Generator ===");
Console.WriteLine($"Output file: {outputFilename}");
Console.WriteLine();

// Load models from JSON file
// Try multiple locations to find cerebras-models.json
string? jsonPath = null;
var searchPaths = new[]
{
    "scripts/db/cerebras/cerebras-models.json",  // From repo root
    "cerebras-models.json",                        // Same directory as script
    Path.Combine(AppContext.BaseDirectory, "cerebras-models.json")  // Script execution directory
};
...
if (jsonPath == null)
{
    Console.WriteLine($"❌ Error: cerebras-models.json not found");
...
var models = new List<CerebrasModel>();
...
foreach (var modelProp in modelsNode.EnumerateObject())
{
    ...
    var model = new CerebrasModel
    {
        ...
        Owner = modelData.GetProperty("owner").GetString() ?? "cerebras",
...
// SQL Generation
static async Task GenerateSQLOutput(List<CerebrasModel> models, string outputFilename)
{
    var sql = new StringBuilder();

    sql.AppendLine("-- Auto-generated SQL for Cerebras models");
    ...
    foreach (var model in models)
    {
        ...
        var standardParameters = "{}"; // Cerebras uses standard OpenAI-compatible parameters
        var description = model.Notes != null ? $"'{EscapeSqlString(model.Notes)}'" : "NULL";
        var modelCardUrl = "https://inference-docs.cerebras.ai/models/overview";
        ...
        sql.AppendLine($"  VALUES ('{EscapeSqlString(model.Owner)}', NULL, 'https://cerebras.ai')");
        ...
        sql.AppendLine($"  ) VALUES (");
        sql.AppendLine($"    v_model_id, '{EscapeSqlString(model.ModelId)}', 9, true,");
    ...
    sql.AppendLine();
    sql.AppendLine($"-- Successfully generated SQL for {models.Count} Cerebras models");
