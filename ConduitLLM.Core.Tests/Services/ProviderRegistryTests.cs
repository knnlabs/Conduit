...
namespace ConduitLLM.Core.Tests.Services
{
    ...
    public class ProviderRegistryTests
    {
        ...
        {
            // Act
            var result = _registry.TryGetMetadata(ProviderType.Anthropic, out var metadata);

            // Assert
            Assert.True(result);
            Assert.NotNull(metadata);
            Assert.Equal(ProviderType.Anthropic, metadata!.ProviderType);
            Assert.Equal("Anthropic", metadata.DisplayName);
        }
        ...
        public void GetAllMetadata_ReturnsAllRegisteredProviders()
        {
            ...
            Assert.True(allMetadata.Count >= 15); // Expecting at least 15 providers
        ...
        public void IsRegistered_WhenProviderExists_ReturnsTrue()
        {
            ...
            Assert.True(_registry.IsRegistered(ProviderType.Anthropic));
            Assert.True(_registry.IsRegistered(ProviderType.AzureOpenAI));
        }
        ...
        public void GetProvidersByFeature_WithStreamingFilter_ReturnsCorrectProviders()
        {
            ...
            Assert.Contains(streamingProviders, p => p.ProviderType == ProviderType.Anthropic);
        }
        ...
        public void GetProvidersByFeature_WithAudioFilter_ReturnsCorrectProviders()
        {
            ...
            Assert.Contains(audioProviders, p => p.ProviderType == ProviderType.GoogleCloud);
        }
        ...
        [Fact]
        public void AnthropicMetadata_HasCorrectConfiguration()
        {
            // Act
            var metadata = _registry.GetMetadata(ProviderType.Anthropic);

            // Assert
            Assert.Equal("https://api.anthropic.com/v1", metadata.DefaultBaseUrl);
            Assert.True(metadata.Capabilities.Features.Streaming);
            Assert.True(metadata.Capabilities.Features.VisionInput);
            Assert.False(metadata.Capabilities.Features.ImageGeneration);
            Assert.True(metadata.Capabilities.ChatParameters.TopK);
            Assert.True(metadata.AuthRequirements.RequiresApiKey);
            Assert.Equal("x-api-key", metadata.AuthRequirements.ApiKeyHeaderName);
        }

        [Fact]
        public void AzureOpenAIMetadata_HasCorrectConfiguration()
        {
            // Act
            var metadata = _registry.GetMetadata(ProviderType.AzureOpenAI);

            // Assert
            Assert.Contains(".openai.azure.com", metadata.DefaultBaseUrl);
            Assert.True(metadata.Capabilities.Features.Streaming);
            Assert.True(metadata.AuthRequirements.RequiresApiKey);
            Assert.True(metadata.AuthRequirements.SupportsOAuth);
            Assert.Equal("api-key", metadata.AuthRequirements.ApiKeyHeaderName);
            
            // Verify custom fields
            var customFields = metadata.AuthRequirements.CustomFields;
            Assert.Contains(customFields, f => f.Name == "resourceName");
            Assert.Contains(customFields, f => f.Name == "deploymentId");
        }

        [Fact]
        public void OllamaMetadata_HasCorrectConfiguration()
        {
            // Act
            var metadata = _registry.GetMetadata(ProviderType.Ollama);

            // Assert
            Assert.Equal("http://localhost:11434/api", metadata.DefaultBaseUrl);
            Assert.False(metadata.AuthRequirements.RequiresApiKey);
            Assert.True(metadata.ConfigurationHints.RequiresSpecialSetup);
        }
        ...
        [Fact]
        public void AzureOpenAI_ValidateConfiguration_RequiresAllFields()
        {
            // Arrange
            var metadata = _registry.GetMetadata(ProviderType.AzureOpenAI);
            var config = new Dictionary<string, object>
            {
                ["apiKey"] = "test-key"
                // Missing resourceName and deploymentId
            };
            ...
            // Assert
            Assert.False(result.IsValid);
            Assert.Contains(result.Errors, e => e.Field == "resourceName");
            Assert.Contains(result.Errors, e => e.Field == "deploymentId");
        }
