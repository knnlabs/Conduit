/**
 * User-friendly error message mapping for OpenAI-compatible error responses.
 *
 * Maps HTTP status codes to titles, messages, suggestions, severity,
 * and recoverability flags. Works with the ConduitError hierarchy
 ...
 */
export function extractRetryAfter(error?: OpenAIError): number | undefined {
  if (error?.message) {
...
/**
 * Get error configuration for a specific HTTP status code.
 */
...
/**
 * Get the severity level for an error based on status code.
 */
