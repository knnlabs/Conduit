...
describe('contract route materialization', () => {
  it('encodes path parameters without changing the contract template registry', () => {
    expect(materializeContractPath('/v1/conduit/tasks/{taskId}', { taskId: 'task/with spaces' }))
      .toBe('/v1/conduit/tasks/task%2Fwith%20spaces');
  });

  it('rejects missing path parameters', () => {
    expect(() => materializeContractPath('/v1/conduit/tasks/{taskId}', {}))
