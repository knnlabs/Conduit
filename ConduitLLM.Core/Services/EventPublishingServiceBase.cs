...
namespace ConduitLLM.Core.Services
{
    ...
    public abstract class EventPublishingServiceBase
    {
        ...
        /// </remarks>
        protected async Task PublishEventAsync<TEvent>(
            TEvent domainEvent,
        ...
        /// <summary>
        /// Publishes a domain event asynchronously with additional context data.
        /// </summary>
        /// <typeparam name="TEvent">The type of event to publish.</typeparam>
        /// <param name="domainEvent">The event to publish.</param>
        /// <param name="operationName">A descriptive name for the operation that triggered the event.</param>
        ...
        protected void LogEventPublishingConfiguration(string serviceName)
        {
            ...
            {
                _logger.LogWarning(
                    "{ServiceName}: Event bus NOT configured - using direct database updates (not recommended for production)",
                    serviceName);
            }
        }
    }
