...
namespace ConduitLLM.Admin.Endpoints
{
    ...
    /// </summary>
    public class BillingAuditEndpoints
    {
        private readonly IBillingAuditService _billingAuditService;
        private readonly ILogger<BillingAuditEndpoints> _logger;
        ...
        /// <summary>
        /// Initializes the Billing Audit endpoint handler.
        /// </summary>
        public BillingAuditEndpoints(
            IBillingAuditService billingAuditService,
            ILogger<BillingAuditEndpoints> logger)
        {
        ...
        public static IEndpointRouteBuilder MapBillingAuditEndpoints(IEndpointRouteBuilder app)
        {
            ...
            group.MapGet("/summary", ([FromServices] BillingAuditEndpoints endpoints, [FromQuery] DateTime? from = null, [FromQuery] DateTime? to = null, [FromQuery] int? virtualKeyId = null) => endpoints.GetSummary(from ?? default, to ?? default, virtualKeyId))
                ...
                .Produces<BillingAuditSummary>()
                .Produces(StatusCodes.Status400BadRequest);
            group.MapGet("/anomalies", ([FromServices] BillingAuditEndpoints endpoints, [FromQuery] DateTime? from = null, [FromQuery] DateTime? to = null) => endpoints.DetectAnomalies(from ?? default, to ?? default))
                .WithName("BillingAudit_DetectAnomalies")
                .Produces<List<BillingAnomaly>>()
                .Produces(StatusCodes.Status400BadRequest);
            ...
            group.MapPost("/export", ([FromServices] BillingAuditEndpoints endpoints, BillingAuditExportRequest request) => endpoints.ExportAuditEvents(request))
                ...
                .Produces(StatusCodes.Status200OK, typeof(void), "application/json", "text/csv")
                .Produces(StatusCodes.Status400BadRequest);
        ...
        public async Task<IResult> QueryAuditEvents(BillingAuditQueryRequest request)
        {
        ...
        public async Task<IResult> DetectAnomalies(
            [FromQuery] DateTime from,
            [FromQuery] DateTime to)
        {
            if (from > to)
        ...
        public async Task<IResult> GetRevenueLoss(
            [FromQuery] DateTime from,
            [FromQuery] DateTime to)
        {
            if (from > to)
        ...
        public async Task<IResult> ExportAuditEvents(BillingAuditExportRequest request)
        {
            if (request.From > request.To)
            {
                return Results.BadRequest("From date must be before or equal to To date");
            ...
            _logger.LogInformation("Exporting {EventCount} billing audit events as {Format} for period {From:O} to {To:O}",
        ...
        private IResult ExportAsJson(List<BillingAuditEvent> events)
        {
            ...
            return Results.File(Encoding.UTF8.GetBytes(json), "application/json", $"billing-audit-{DateTime.UtcNow:yyyyMMddHHmmss}.json");
        }

        private IResult ExportAsCsv(List<BillingAuditEvent> events)
        {
            ...
            return Results.File(Encoding.UTF8.GetBytes(csv.ToString()), "text/csv", $"billing-audit-{DateTime.UtcNow:yyyyMMddHHmmss}.csv");
        }
