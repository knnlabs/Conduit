...
public sealed class OperationIdValidationDocumentTransformer : IOpenApiDocumentTransformer
{
    ...
    public static void Validate(OpenApiDocument document)
    {
        ...
        foreach (var path in document.Paths ?? new OpenApiPaths())
        {
            ...
            foreach (var operationEntry in path.Value.Operations ?? new Dictionary<System.Net.Http.HttpMethod, OpenApiOperation>())
            {
                ...
                if (string.IsNullOrWhiteSpace(operationId))
                    throw new InvalidOperationException($"OpenAPI operation {location} has no operationId.");
                if (!seen.TryAdd(operationId, location))
                    throw new InvalidOperationException($"Duplicate OpenAPI operationId '{operationId}' on {seen[operationId]} and {location}.");
            }
        }
    }
}
