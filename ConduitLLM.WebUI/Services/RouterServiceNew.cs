...
{
    /// <summary>
    /// Repository-based service that manages the LLM router configurations and provides access to the router
    /// </summary>
    public class RouterServiceNew : IRouterService
    {
        ...
        /// <summary>
        /// Creates a new instance of the RouterServiceNew using repository pattern
        /// </summary>
        public RouterServiceNew(
            ...
            ILogger<RouterServiceNew> logger,
        ...
        public async Task<RouterConfig> GetRouterConfigAsync()
        {
            try
            {
                ...
                var globalSetting = await _globalSettingRepository.GetByKeyAsync(ROUTER_CONFIG_KEY);
                ...
                if (globalSetting != null)
                {
                    try
                    {
                        ...
                        var config = JsonSerializer.Deserialize<RouterConfig>(globalSetting.Value);
                        if (config != null)
                        {
                            return config;
                        }
            ...
            {
                _logger.LogError(ex, "Error getting router configuration");
                return new RouterConfig();
            }
        }
        ...
        /// <summary>
        /// Saves the router configuration using the repository
        /// </summary>
        private async Task SaveRouterConfigAsync(RouterConfig config)
        {
            ...
            // Check if we already have a setting and update or create it
            await _globalSettingRepository.UpsertAsync(
                ROUTER_CONFIG_KEY, 
                serializedConfig, 
                "Router configuration including model deployments and fallbacks");
        }
