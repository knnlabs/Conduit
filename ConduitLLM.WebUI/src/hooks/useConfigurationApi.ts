...
export function useConfigurationApi() {
  ...
  const getRoutingSettings = useCallback(async (): Promise<RoutingSettings> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error ?? 'Failed to fetch routing settings');
      }

      return result as RoutingSettings;
    } catch (err) {
  ...
  const updateRoutingSettings = useCallback(async (settings: Partial<RoutingSettings>): Promise<RoutingSettings> => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = result as { error?: string };
        throw new Error(errorData.error ?? 'Failed to update routing settings');
      }
  ...
  const getCachingSettings = useCallback(async (): Promise<CachingSettings> => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = result as { error?: string };
        throw new Error(errorData.error ?? 'Failed to fetch caching settings');
      }

      return result as CachingSettings;
    } catch (err) {
  ...
  const updateCachingSettings = useCallback(async (settings: Partial<CachingSettings>): Promise<CachingSettings> => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = result as { error?: string };
        throw new Error(errorData.error ?? 'Failed to update caching settings');
      }
  ...
  const getCacheStats = useCallback(async (cacheId?: string): Promise<CacheStats> => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = result as { error?: string };
        throw new Error(errorData.error ?? 'Failed to fetch cache stats');
      }
